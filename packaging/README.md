...
- `linux/scheme-handler.desktop` - Linux desktop entry used by DEB/AppImage.

The Linux desktop entry uses `Icon=scheme-handler`; the GitHub Actions workflow copies `packaging/icon.png` into the standard AppDir hicolor icon directory before running `linuxdeploy`.
